...
export default function Topbar({ sectionTitle, sectionSubtitle }: TopbarProps) {
    ...
    <header className="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200/80 bg-white/90 px-6 backdrop-blur-md">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        <div className="flex items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5">
          ...
          <span className="hidden sm:block text-xs font-semibold text-emerald-700">Live</span>
        </div>
          ...
          {notifOpen && (
            <div className="absolute right-0 mt-2 w-80 rounded-xl border border-slate-200 bg-white shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1)] overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                ...
                <span className="rounded-full bg-[#800000]/10 px-2 py-0.5 text-xs font-semibold text-[#800000]">3 new</span>
              </div>
              <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
                {[
                  { icon: "✓", text: "Admin 'Rahul Sharma' approved", time: "2m ago", color: "text-emerald-600" },
                  { icon: "⚠", text: "System sync completed for 45 classes", time: "15m ago", color: "text-amber-600" },
                  { icon: "🔔", text: "Batch 'SY-BCA-2024' created", time: "1h ago", color: "text-blue-600" },
                ].map((n, i) => (
                  <div key={i} className="flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition-colors cursor-pointer">
                    <span className={`mt-0.5 flex-shrink-0 text-sm ${n.color}`}>{n.icon}</span>
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            ...
            <span className="hidden sm:block text-xs font-semibold text-slate-700">Super Admin</span>
          ...
          {profileOpen && (
            <div className="absolute right-0 mt-2 w-56 rounded-xl border border-slate-200 bg-white shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1)] overflow-hidden">
              <div className="border-b border-slate-100 px-4 py-3">
                ...
                <p className="text-xs text-slate-400">superadmin@indira.edu.in</p>
              </div>
              <div className="p-1">
                {[
